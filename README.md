...
**Offer捕手** 是一个面向高校学生的AI求职参谋系统，突破传统"简历打分器"的局限，打造“岗位发现 → JD解码 → 简历优化 → 面试准备”的求职赋能工具：

本项目对应 Demo 课题 **作业一：「Offer捕手」学生求职匹配智能体**，核心目标是帮助学生有效匹配心仪岗位，并提升简历初筛命中率。

| 模块 | 功能 | 创新点 |
|------|------|--------|
| 🧭 **岗位猎手** | AI岗位画像 + 最新岗位分页搜集 + 岗位匹配排序 | 先根据简历推荐细分方向，再把最新岗位按匹配度带回流程 |
...
├──────────┼──────────┼────────┼────────────────┤
│AI岗位画像 │JD深度解码 │逆向能力建模│高频题预测  │
│简历方向推荐│HR视角模拟 │差距雷达图  │STAR生成    │
│多平台检索 │公司情报   │一键重写    │模拟面试    │
└──────────┴──────────┴────────┴────────────────┘
```
...
- **简历解析**：pdfplumber
- **岗位搜集**：requests + 腾讯招聘公开岗位查询接口 + 多公司官方入口
- **图表**：Plotly（雷达图）
- **文档导出**：python-docx
...
### 方案设计
Offer捕手以**逆向求职**为核心思路——不是"我的简历能匹配什么"，而是"我想去哪里，现在差什么，怎么弥补"。四大模块形成闭环：岗位猎手先调用 AI 根据简历生成细分岗位画像，分页搜集最新岗位并做匹配排序，情报官帮助学生读懂JD本质，简历军师量化差距并直接产出优化简历，面试教练提供从预测题到模拟面试的完整准备。
...
- **岗位数据**：调用腾讯招聘公开岗位查询接口，并补充牛客、实习僧、BOSS直聘等外部检索入口

### 创新点
1. **简历驱动岗位推荐**：先从简历识别可投方向，再搜集最新岗位，解决“海投不知道投什么”的入口问题
2. **HR视角模拟**：模拟HR看简历时的真实内心OS，6秒扫描→细读→初筛决策全流程
3. **JD潜台词解码**：逐条翻译JD字面要求背后的真实考查意图
4. **逆向能力建模**：从JD反推五维能力雷达图，量化差距
5. **一键重写简历**：不是给建议，是直接产出可下载的优化版简历
